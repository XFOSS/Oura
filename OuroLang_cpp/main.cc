    ...
    while (!at_end()) {
      ...
      if (auto token_result = scan_token(); token_result) {
        ...
        return std::unexpected{
            std::format("Lexical error at {}:{}", line_, column_)};
      }
    }
...
private:
  ...
  [[nodiscard]] auto scan_identifier(std::size_t start)
    ...
    const auto token_type =
        ...
        std::ranges::to<std::optional>() | std::views::transform([](auto opt) {
          return opt.value_or(TokenType::Identifier);
        });

    return Token{*token_type.begin(), lexeme, line_, column_};
  }

  [[nodiscard]] auto scan_number(std::size_t start)
      -> std::expected<Token, std::string> {
    while (!at_end() && (std::isdigit(peek()) || peek() == '.'))
      advance();
    return Token{TokenType::Number, source_.substr(start, position_ - start),
                 line_, column_};
  }

  [[nodiscard]] auto scan_punctuation(char c)
      -> std::expected<Token, std::string> {
    ...
    return token_type
        .transform(
            [&](auto type) { return Token{type, single_char, line_, column_}; })
        .or_else([&]() -> std::expected<Token, std::string> {
          return std::unexpected{std::format("Unknown character '{}' at {}:{}",
                                             c, line_, column_)};
        });
  }
};
...
export struct NumberExpr {
  double value;
};
export struct IdentifierExpr {
  std::string name;
};
...
public:
  explicit constexpr Parser(std::span<const Token> tokens) noexcept
      : tokens_{tokens} {}
...
private:
  ...
  [[nodiscard]] auto parse_statement() -> std::expected<Stmt, ParseError> {
    ...
    return std::unexpected{
        ParseError{"Expected statement", peek().line, peek().column}};
  }

  [[nodiscard]] auto parse_var_declaration()
      -> std::expected<VarDeclStmt, ParseError> {
    return consume(TokenType::Identifier, "Expected variable name")
        .and_then([&](const auto &name_token)
                      -> std::expected<VarDeclStmt, ParseError> {
          ...
          return consume(TokenType::Equals, "Expected '=' after variable name")
              ...
              .and_then(
                  [&](auto expr) -> std::expected<VarDeclStmt, ParseError> {
                    return consume(TokenType::Semicolon,
                                   "Expected ';' after variable declaration")
                        .transform([&](auto) {
                          return VarDeclStmt{
                              std::move(name), std::move(type_annotation),
    ...
    return consume(TokenType::LParen, "Expected '(' after 'if'")
        ...
        .and_then([&](auto condition) -> std::expected<IfStmt, ParseError> {
          return consume(TokenType::RParen, "Expected ')' after if condition")
              .and_then([&](auto) {
                return consume(TokenType::LBrace,
                               "Expected '{' before if body");
              })
              .and_then([&](auto) { return parse_block(); })
              .and_then([&](auto then_branch)
                            -> std::expected<IfStmt, ParseError> {
  ...
  [[nodiscard]] auto parse_return_statement()
      -> std::expected<ReturnStmt, ParseError> {
    return parse_expression().and_then(
        [&](auto value) -> std::expected<ReturnStmt, ParseError> {
          return consume(TokenType::Semicolon,
  ...
  [[nodiscard]] auto parse_block()
      -> std::expected<std::vector<Stmt>, ParseError> {
    auto statements = std::vector<Stmt>{};
  ...
  [[nodiscard]] auto parse_addition() -> std::expected<Expr, ParseError> {
    return parse_multiplicative().and_then(
        [&](auto expr) -> std::expected<Expr, ParseError> {
          ...
          while (match<TokenType::Plus, TokenType::Minus>()) {
            ...
            if (auto right_result = parse_multiplicative()) {
              result = BinaryExpr{
                  operator_type, std::make_unique<Expr>(std::move(result)),
  ...
  [[nodiscard]] auto parse_primary() -> std::expected<Expr, ParseError> {
      ...
      return parse_expression().and_then(
          [&](auto expr) -> std::expected<Expr, ParseError> {
            return consume(TokenType::RParen, "Expected ')' after expression")
                .transform([expr = std::move(expr)](auto) mutable {
    ...
    return std::unexpected{
        ParseError{"Expected expression", peek().line, peek().column}};
  }
};
...
concept Analyzable = requires(T t) { t.accept_analyzer(std::declval<auto>()); };
...
public:
  [[nodiscard]] auto analyze(const std::vector<Stmt> &statements)
    ...
    return statements | std::views::transform([this](const auto &stmt) {
             return analyze_statement(stmt);
           }) |
           std::ranges::to<std::vector>() |
           std::views::filter(
               [](const auto &result) { return !result.has_value(); }) |
           std::views::transform(
               [](const auto &result) { return result.error(); }) |
           std::ranges::to<std::vector>() |
           std::views::take(1) // First error only
           | std::ranges::to<std::optional>() |
           std::views::transform(
               [](const auto &errors) -> std::expected<void, std::string> {
                 return errors.empty() ? std::expected<void, std::string>{}
...
private:
  [[nodiscard]] auto analyze_statement(const Stmt &stmt)
      -> std::expected<void, std::string> {
    return std::visit([this](const auto &s) { return analyze_impl(s); }, stmt);
  }

  [[nodiscard]] auto analyze_impl(const VarDeclStmt &stmt)
      -> std::expected<void, std::string> {
        ...
        .and_then([&](auto inferred_type) -> std::expected<void, std::string> {
          if (stmt.type_annotation) {
            ...
            if (declared_type != inferred_type &&
              ...
              return std::unexpected{
                  std::format("Type mismatch for variable '{}': declared as "
                              "{}, inferred as {}",
                              stmt.name, type_to_string(declared_type),
  ...
  [[nodiscard]] auto analyze_impl(const IfStmt &stmt)
      -> std::expected<void, std::string> {
    return infer_type(*stmt.condition)
        .and_then([&](auto condition_type) -> std::expected<void, std::string> {
          ...
          const auto analyze_branch = [this](const auto &branch) {
            return branch | std::views::transform([this](const auto &s) {
                     return analyze_statement(s);
                   }) |
                   std::ranges::to<std::vector>() |
                   std::views::filter(
                       [](const auto &r) { return !r.has_value(); }) |
                   std::views::transform(
                       [](const auto &r) { return r.error(); }) |
                   std::ranges::to<std::vector>() | std::views::take(1) |
                   std::ranges::to<std::optional>() |
                   std::views::transform(
                       [](const auto &errors)
                           -> std::expected<void, std::string> {
          ...
          return analyze_branch(stmt.then_branch).and_then([&](auto) {
            return analyze_branch(stmt.else_branch);
          });
        });
  }

  [[nodiscard]] auto analyze_impl(const ReturnStmt &stmt)
      -> std::expected<void, std::string> {
    return infer_type(*stmt.value).transform([](auto) {});
  }

  [[nodiscard]] auto infer_type(const Expr &expr)
      -> std::expected<Type, std::string> {
    return std::visit([this](const auto &e) { return infer_type_impl(e); },
                      expr);
  }

  [[nodiscard]] constexpr auto
  infer_type_impl(const NumberExpr &) const noexcept
      -> std::expected<Type, std::string> {
    return Type::Float;
  }
  ...
  string_to_type(std::string_view type_name) noexcept -> Type {
    if (type_name == "int")
      return Type::Integer;
    if (type_name == "float")
      return Type::Float;
  ...
  [[nodiscard]] static constexpr auto type_to_string(Type type) noexcept
      -> std::string_view {
    switch (type) {
    case Type::Integer:
      return "int"sv;
  ...
  auto execute(std::string_view source) -> void {
    const auto process_pipeline = [&]() -> std::expected<void, std::string> {
      ...
      return lexer.tokenize()
          .and_then([&](auto tokens)
                        -> std::expected<std::vector<Stmt>, std::string> {
            auto parser = Parser{tokens};
            return parser.parse().transform_error(
                [](const auto &error) { return std::string{error.what()}; });
          })
          .and_then([&](auto ast) -> std::expected<void, std::string> {
            auto checker = TypeChecker{};
            return checker.analyze(ast).and_then(
                [&, ast = std::move(ast)](
                    auto) mutable -> std::expected<void, std::string> {
...
private:
  [[nodiscard]] auto execute_statement(const Stmt &stmt)
      -> std::expected<void, std::string> {
    return std::visit([this](const auto &s) { return execute_impl(s); }, stmt);
  }

  [[nodiscard]] auto execute_impl(const VarDeclStmt &stmt)
      -> std::expected<void, std::string> {
    return evaluate_expression(*stmt.value).transform([&](auto value) {
      environment_[stmt.name] = std::move(value);
    });
  }

  [[nodiscard]] auto execute_impl(const IfStmt &stmt)
      -> std::expected<void, std::string> {
    return evaluate_expression(*stmt.condition)
        .and_then(
            [&](auto condition_value) -> std::expected<void, std::string> {
              const auto is_truthy = std::visit(
              ...
              const auto &branch =
                  is_truthy ? stmt.then_branch : stmt.else_branch;
  ...
  [[nodiscard]] auto execute_impl(const ReturnStmt &stmt)
      -> std::expected<void, std::string> {
    return evaluate_expression(*stmt.value).transform([](auto) {});
  }

  [[nodiscard]] auto evaluate_expression(const Expr &expr)
      -> std::expected<Value, std::string> {
    return std::visit([this](const auto &e) { return evaluate_impl(e); }, expr);
  }

  [[nodiscard]] constexpr auto
  evaluate_impl(const NumberExpr &expr) const noexcept
      -> std::expected<Value, std::string> {
  ...
  [[nodiscard]] auto evaluate_impl(const BinaryExpr &expr)
      -> std::expected<Value, std::string> {
    return evaluate_expression(*expr.left)
        .and_then([&](auto left_val) -> std::expected<Value, std::string> {
          return evaluate_expression(*expr.right)
              .and_then(
                  [&](auto right_val) -> std::expected<Value, std::string> {
                    if (std::holds_alternative<double>(left_val) &&
                        std::holds_alternative<double>(right_val)) {

                      const auto [l, r] =
                          std::make_pair(std::get<double>(left_val),
                      ...
                      return compute().transform(
                          [](auto result) { return Value{result}; });
                    }
...
public:
  auto run() -> void {
    ...
    for (std::string input;
         std::print("ouro> "), std::getline(std::cin, input);) {
...
//! Application entry point with modern exception handling and coroutines
//! support
int main() {
  try {
